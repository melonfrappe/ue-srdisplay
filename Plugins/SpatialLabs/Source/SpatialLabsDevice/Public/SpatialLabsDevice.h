...
#include "SpatialLabsDevice_Layer.h"
...
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnSpatialLabsRenderTextureCompletedDelegate, FRHICommandListImmediate&, FTexture2DRHIRef);

/**
 * Simple Head Mounted Display
 */
class FSpatialLabsDevice 
...
public:
	...
	float GetScaledWorldToMetersScale() const;
...
public:
	/** Constructor */
	FSpatialLabsDevice(const FAutoRegister&, XrInstance InInstance, XrSystemId InSystem, TRefCountPtr<FOpenXRRenderBridge>& InRenderBridge, TArray<const char*> InEnabledExtensions, TArray<class IOpenXRExtensionPlugin*> InExtensionPlugins);

	/** Destructor */
	virtual ~FSpatialLabsDevice();
	...
	SPATIALLABSDEVICE_API bool IsInitialized() const;
	SPATIALLABSDEVICE_API bool IsRunning() const;
	SPATIALLABSDEVICE_API bool IsFocused() const;

	SPATIALLABSDEVICE_API int32 AddActionDevice(XrAction Action, XrPath Path);
	SPATIALLABSDEVICE_API void ResetActionDevices();

	XrPath GetTrackedDevicePath(const int32 DeviceId);

	SPATIALLABSDEVICE_API bool IsExtensionEnabled(const FString& Name) const { return EnabledExtensions.Contains(Name); }
	SPATIALLABSDEVICE_API XrInstance GetInstance() { return Instance; }
	SPATIALLABSDEVICE_API XrSystemId GetSystem() { return System; }
	SPATIALLABSDEVICE_API XrSession GetSession() { return Session; }
	SPATIALLABSDEVICE_API XrSpace GetTrackingSpace()
	{
		return (TrackingSpaceType == XR_REFERENCE_SPACE_TYPE_STAGE) ? StageSpace : LocalSpace;
	}
	SPATIALLABSDEVICE_API XrTime GetDisplayTime() const;
	SPATIALLABSDEVICE_API TArray<IOpenXRExtensionPlugin*>& GetExtensionPlugins() { return ExtensionPlugins; }
	SPATIALLABSDEVICE_API bool IsStandaloneStereoOnlyDevice() const { return bIsStandaloneStereoOnlyDevice; }

	SPATIALLABSDEVICE_API FOnSpatialLabsRenderTextureCompletedDelegate& GetOnSpatialLabsRenderTextureCompletedDelegate() { return OnSpatialLabsRenderTextureCompletedDelegate; }
...
private:
	...
	FOnSpatialLabsRenderTextureCompletedDelegate OnSpatialLabsRenderTextureCompletedDelegate;

public:
	void AddWidget(UUserWidget* Widge, int32 ZOrder);
	void RemoveWidget(UUserWidget* Widget);
	void ClearWidgets(UWorld* World);

	void ViewportResized(FViewport* Viewport, uint32);
	void OnTick(UWorld* World, ELevelTick TickType, float DeltaTime);

private:

	struct ViewportLetterBox
	{
		UGameViewportClient* Viewport;
		TSharedPtr<SVerticalBox> LetterBox;
		TSharedPtr<SBox> LetterBoxContent;
		TSharedPtr<SConstraintCanvas> LetterBoxContentLayout;
		TSharedPtr<SConstraintCanvas> BoxControl;
		TSharedPtr<SConstraintCanvas> BoxLeft;
		TSharedPtr<SConstraintCanvas> BoxRight;
	};
	...
	TMap<UGameViewportClient*, ViewportLetterBox> ViewportLetterBoxes;
	TArray<ViewportLetterBox*> ViewportLetterList;
	TArray<UserWidget> PendingUserWidgets;

	void SetupLetterBoxWidget(UGameViewportClient*);
	void ResizeLetterboxes();
	void CleanupLetterbox(bool);
	void AppendWidgetsToLetterbox(UGameViewportClient*);

	float FOVPlayerCamera = 90;
	float FocalLengthPlayerCamera_CM = 40;
	bool UseDynamicCameraFOVAroundSceenCenter = true;

	void UpdatePlayerCameraData();

	static float WorldToMetersCache;

public:

	static FVector GetCachedEyeLocation(EStereoscopicPass StereoPassType);
	static FVector GetCachedEyeScreenSpaceLocation(EStereoscopicPass StereoPassType);
};
